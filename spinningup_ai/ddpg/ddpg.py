...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    # Action limit for clamping: critically assumes that all the actions have tyhe same bounds
    act_limit = env.action_space.high[0]

    ac = actor_critic(env.observation_space, env.action_space, **ac_kwargs)
    ac_targ = deepcopy(ac)

    # Freeze the target networks with respect to the optimizers (only update via polyak averaging)
    ...
    # count the variables (protip: try to get a feel of how different size networks behave)
    ...
    def update(data):
        ...
        # next run one step gradient descent for pi
        ...
        # Finaaly update the target parameters with polyak averaging
        with torch.no_grad():
    ...
    for t in range(total_steps):
        ...
        # store expereience in the replay buffer
        replay_buffer.store(o, a, r, o2, d)

        o = o2

        # end trajectory handling
